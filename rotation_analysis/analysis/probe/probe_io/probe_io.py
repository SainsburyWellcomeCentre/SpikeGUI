...
from rotation_analysis.analysis.probe.probe_io.io_exceptions import IgorFileKeyError, BonsaiQueryError
from rotation_analysis.analysis.stimulus import DEGREES_PER_VOLT
...
class TriggerTraceIo(ProbeIo):
    ...
    def get_corrected_trigger(self, idx, verbose=True):
        return self.get_trigger(idx) + self.get_photodiode_offset(idx, verbose)

    def get_photodiode_offset(self, idx, verbose=True):
        #
        ...
        trace_blk = self.photodiode_trigger_trace[this_trigger_onset:next_trigger_onset]

        # determines the index of the first valid value in the block. if none, then return 0
        i = next((i for i in range(len(trace_blk)) if trace_blk[i]), 0)

        # outputs the message to screen (if required)
        if verbose:
            if i > 0:
...
class IgorIo(ProbeIo):
    ...
    def get_waveforms_in_condition_group(self, condition_grp):

        #
        all_grp_waveforms = []
        for ck in condition_grp:
            all_grp_waveforms.append(self.get_waveforms_in_condition(ck))

        # combines all the waveform signals into a singal array
        return cf.flat_list(all_grp_waveforms)
