...
def plot_cell(trace, trace_label, delta_wave, events_pos, peaks_pos, half_rises):
    fig = plt.figure()
...
def detect_cell(profile, cell_id, threshold, n_pnts_bsl, n_pnts_peak, n_pnts_rise_t, n_pnts_for_peak_detection,
                n_sds, n_pnts_high_pass_filter, minimum_time_delta=60):
    if profile.ndim == 3:
        trace = (profile[:, cell_id, 0]).copy()
    else:
        trace = (profile[:, cell_id]).copy()
    return detect_trace(trace,
                        threshold,
                        n_pnts_bsl,
                        n_pnts_peak,
                        n_pnts_rise_t,
...
def main(args_list):
    ...
    n_pnts_bsl, n_pnts_peak, n_pnts_rise_t = [int(arg) for arg in args_list[2:5]]
...
#    for i in range(2):        
    for j in range(green_profiles.shape[1]):
        ...
        events_pos, peaks_pos, half_rises, peak_ampls = detect_cell(profile,
                                                                    j,
                                                                    threshold,
                                                                    n_pnts_for_peak_detection,
                                                                    n_sds,
                                                                    n_pnts_high_pass_filter)
        # PLOT
        delta_wave = diff(box_smooth(profile, n_pnts_peak), box_smooth(profile, n_pnts_bsl), n_pnts_rise_t)
        plot_cell(profile, 'GreenCell{}'.format(j), delta_wave, events_pos, peaks_pos, half_rises)
