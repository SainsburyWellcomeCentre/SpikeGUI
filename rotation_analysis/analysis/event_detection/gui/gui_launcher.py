...
from analysis.event_detection.gui.detection_interface import DetectionInterface
...
def get_main_window(app_engine):
    if sys.platform == "win32":
        base_dir = os.path.abspath(os.path.normpath('./analysis/event_detection/gui/'))  # FIXME: should be conf based
    else:
        base_dir = os.path.abspath(os.path.normpath('./event_detection/gui/'))  # FIXME: should be conf based
    qml_source_path = os.path.join(base_dir, 'qml', 'Detection', 'main.qml')
    if not os.path.isfile(qml_source_path):
        raise GuiError("Qml code not found at {}, please verify your installation".format(qml_source_path))
...
def main(blocks=None, angle=None):
    ...
    detector = DetectionInterface(app, context, win, display_name="detectionGraph",
                                  image_provider=analysis_image_provider, image_provider_name="analysisprovider")
