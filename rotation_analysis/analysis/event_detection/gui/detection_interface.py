...
class DetectionInterface(QObject):
    ...
    def __init__(self, app, context, parent, display_name, image_provider, image_provider_name):
        """
    ...
    def current_params(self):
        ...
        :return:
        :rtype: analysis.event_detection.detection_params.DetectionParams
        """
    ...
    def detect(self):
        ...
        plt.clf()
        self.current_block.plotter.plot_single_trial_detection(self.angle, self.current_trial_id)
    ...
    def get_average_amplitude(self):
        if self.current_params:
            ampl = self.current_block.trials[self.current_trial_id].events.average_amplitude()
            return '{0:.2f}'.format(ampl)

    @pyqtSlot(result=str)
    def get_integral(self):
        if self.current_params:
            integral = self.current_block.get_events_integral(self.angle, self.current_trial_id)
