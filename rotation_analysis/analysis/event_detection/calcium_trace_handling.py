...
def load_fluo_profiles(profiles_path):
    ...
    neuropiles_idx = [i for i in range(len(header)) if header[i].startswith("neuropile")]
    ...
    if cells_idx and neuropiles_idx:
        if not cells.size == neuropiles.size:
            raise ValueError("Cells and neuropiles must have same length in {}".format(profiles_path))
        data = np.dstack((cells, neuropiles))  # concatanate cells and neuropile in z
    else:
        if cells_idx:
            data = cells
        elif neuropiles_idx:
