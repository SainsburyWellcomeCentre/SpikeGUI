import numpy as np
from margrie_libs.margrie_libs.signal_processing.thresholding import find_level_decrease
...
class Event(object):
    ...
    def __init__(self, start_p, peak_p, half_rise_p, amplitude, sampling_interval, default_comparator='peak_p'):
    ...
    def get_integral(self, trace):
        try:
            ...
            end_p = find_level_decrease(trace[self.peak_p:],
                                        start_p_y + self.amplitude * 0.2)  # return to 20% amplitude
            end_p += self.peak_p
