...
class DetectionParams(object):
    ...
    def __init__(self, config_file_path=None):
        ...
        self.n_pnts_for_peak_detection = self.conf['event_template']['n_pnts_for_peak_detection']

        self.n_sds = self.conf['sds']['n_sds']

        self.n_pnts_high_pass_filter = self.conf['filtering']['n_pnts_high_pass_filter']
    ...
    def from_conf(self, config_file_path):
        self.config_path = os.path.normpath(os.path.expanduser(config_file_path))
        cfg = ConfigObj(self.config_path, encoding="UTF8", indent_type=' ' * 4, unrepr=True,
                        create_empty=True, write_empty_values=True)
        cfg.reload()
        self.conf = cfg
    ...
    def from_csv(self, config_file_path):
        ...
        self.conf = ConfigObj(self.config_path, encoding="UTF8", indent_type=' ' * 4, unrepr=True,
                              create_empty=True, write_empty_values=True)  # TEST:
        ...
        self.conf['event_template']['n_pnts_for_peak_detection'] = df['n_pnts_for_peak_detection'][0]
        self.conf['sds'] = {}
        self.conf['sds']['n_sds'] = df['n_sds'][0]
        self.conf['filtering'] = {}
        self.conf['filtering']['n_pnts_high_pass_filter'] = df['n_pnts_high_pass_filter'][0]
        self.conf['filtering']['median_kernel_size'] = df['median_kernel_size'][0]
    ...
    def save_event_template_params(self):
        ...
        evt_template['n_pnts_for_peak_detection'] = self.n_pnts_for_peak_detection

        self.save_conf()

    def save_filtering_params(self):
        filtering_params = self.conf['filtering']
        filtering_params['n_pnts_high_pass_filter'] = self.n_pnts_high_pass_filter
    ...
    def to_df(self):
        return pd.DataFrame(
            ...
            columns=('n_pnts_high_pass_filter',
                     'median_kernel_size',
                     'threshold',
                     'n_pnts_bsl',
                     'n_pnts_peak',
