...
from margrie_libs.margrie_libs.signal_processing.thresholding import find_levels, find_level_increase
from margrie_libs.margrie_libs.utils.print_utils import dprint


def _detect_events(trace, n_pnts_bsl, n_pnts_peak, n_pnts_rise_t, threshold, min_n_pnts_from_start=10):
    ...
    if len(events) > 0:
        if events[0] < min_n_pnts_from_start:  # Remove first one if too close to start
            events = events[1:]
    else:
        raise StopIteration("No events found with parameters: {}, {}, {}, {}"
                            .format(n_pnts_bsl, n_pnts_peak, n_pnts_rise_t, threshold))
    return events
...
def find_events_half_rises(trace, events_pos, peaks_pos):
    ...
    for i, bounds in enumerate(zip(events_pos, peaks_pos)):
        try: # FIXME: better error handling
            ...
            half_rise_loc = find_level_increase(event_rise, event_rise[0] + (event_ampl / 2.0))
        except IndexError:
...
def find_events_amplitudes(trace, events_pos, peaks_pos):
    ...
    for i, bounds in enumerate(zip(events_pos, peaks_pos)):
        event_start, event_end = bounds
        ampls[i] = trace[event_end] - trace[event_start]  # TODO: do for both pos and neg
    return ampls
...
def remove_refractory_period(trace, delta_wave, events_pos, half_rises, peaks_pos, n_pnts_refractory_period):
...
def filter_duplicates(events_pos, minimum_time_delta):
    ...
    only the first one
    """
    dprint('{} events before remove duplicates, {}'.format(len(events_pos), events_pos))
    good_indices = []
    try:
    ...
    for i in reversed_idx:
        if i == 0:  # Last event
            ...
            if (events_pos[i] - events_pos[i-1]) > minimum_time_delta:
                good_indices.append(i)
    dprint('{} events after remove duplicates, {}'.format(len(good_indices), events_pos[good_indices]))
...
def _filter_event_start_duplicates(events_pos, minimum_time_delta):
    ...
    if len(good_events):
        ...
        print('0 events after filtering by baseline time, before: {}'.format(events_pos))
    return events_pos
...
def _filter_peak_duplicates(events_pos, peaks_pos, minimum_time_delta):
    ...
    if len(good_events):
        ...
        dprint('0 events after filtering by peak time. Before, bsl: {}, peak: {}'.format(events_pos, peaks_pos))
    return events_pos, peaks_pos


def _sd_filter(events_pos, n_pnts_high_pass_filter, n_sds, peak_ampls, peaks_pos, trace):
    ...
    if len(good_events):
        ...
        dprint("SD removed {} events ({} to {})".format(n_events_before_sd - n_events_after_sd, n_events_before_sd,
                                                        n_events_after_sd))
        peaks_pos = peaks_pos[good_events]
...
def detect_trace(trace, threshold, n_pnts_bsl, n_pnts_peak, n_pnts_rise_t, n_pnts_for_peak_detection,
                 n_sds, n_pnts_high_pass_filter, median_kernel_size=9, minimum_time_delta=60):  # FIXME: some \n left in trace

    trace, n_pnts_refractory_period = add_refractory_period(n_pnts_bsl, n_pnts_peak, n_pnts_rise_t, trace)  # FIXME: may not be correct NAME

    trace = signal.medfilt(trace, median_kernel_size)
    delta_wave = diff(box_smooth(trace, n_pnts_peak), box_smooth(trace, n_pnts_bsl), n_pnts_rise_t)
    default_result = trace, delta_wave, False, False, False, False

    # DETECT
    events_start = _detect_events(trace, n_pnts_bsl, n_pnts_peak, n_pnts_rise_t, threshold)

    # FILTER
    events_start = _filter_event_start_duplicates(events_start, minimum_time_delta)
    if not len(events_start):
        return default_result

    peaks_pos = find_events_peaks(trace, events_start, n_pnts_for_peak_detection)
    events_start, peaks_pos = _filter_peak_duplicates(events_start, peaks_pos, minimum_time_delta)
    if not len(events_start):
        return default_result

    peak_ampls = find_events_amplitudes(trace, events_start, peaks_pos)

    events_start, peak_ampls, peaks_pos = _sd_filter(events_start, n_pnts_high_pass_filter, n_sds,
                                                     peak_ampls, peaks_pos, trace)
    if not len(events_start):
        return default_result

    half_rises = find_events_half_rises(trace, events_start, peaks_pos)

    trace, delta_wave, events_start, half_rises, peaks_pos = remove_refractory_period(trace, delta_wave, events_start,
                                                                                      half_rises, peaks_pos,
